...
public class ConfirmPage extends JFrame {
	 ...
	 * Create the frame.
	 */
	public ConfirmPage(Ticket ticket, Service service) {
		...
		JLabel lblNewLabel = new JLabel("Xác nhận thông tin đơn hàng của bạn và thanh toán");
		lblNewLabel.setForeground(Color.BLUE);
		lblNewLabel.setFont(new Font("Tahoma", Font.PLAIN, 18));
		lblNewLabel.setBounds(276, 21, 452, 48);
		...
		btnNewButton.setBounds(732, 576, 135, 42);
		...
		btnNewButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					ticketRepository.executeQuery("UPDATE ticket SET status=200 WHERE flightId="+flightId+ " AND seatId="+ seatId);
					Receipt receipt = new Receipt();
					receipt.setUserId(8);
					receipt.setPayment(ticket.getCost()+service.getCost());
					receipt.setServiceId(service.getId());
					receipt.setTicketId(flightId);
					receiptRepository.insertEntity(receipt);
					ShowReceipt showReceipt = new ShowReceipt(ticket, service);
